...
if old_open_add in code:
    code = code.replace(old_open_add, new_open_add)
...
if old_add_url in code:
    code = code.replace(old_add_url, new_add_url)
...
if old_delete_extra in code:
    code = code.replace(old_delete_extra, new_delete_extra)
...
if old_add_var in code:
    code = code.replace(old_add_var, new_add_var)
...
if old_del_var in code:
    code = code.replace(old_del_var, new_del_var)
...
if old_sel_all in code:
    code = code.replace(old_sel_all, new_sel_all)
...
if old_bulk_del in code:
    code = code.replace(old_bulk_del, new_bulk_del)
...
if old_bulk_edit in code:
    code = code.replace(old_bulk_edit, new_bulk_edit)
    print("Updated applyBulkEdit successfully!")
else:
    print("WARNING: applyBulkEdit not found!")

# Save back to file with Windows endings
code = code.replace("\n", "\r\n")

with open(file_path, "w", encoding="utf-8") as f:
    f.write(code)

print("All formatted replacements applied successfully!")
