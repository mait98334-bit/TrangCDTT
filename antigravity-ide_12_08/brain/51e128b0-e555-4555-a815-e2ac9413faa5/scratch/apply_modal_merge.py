...
if "activeTabInModal" not in code:
    code = code.replace(
    ...
    print("Added activeTabInModal state.")

# Replace handleOpenAdd and handleOpenEdit
old_open_helpers = """    // Mở modal Thêm sản phẩm chính
    const handleOpenAdd = () => {
        setModalType('add');
        setFormData({
            id: '',
...
new_open_helpers = """    // Mở modal Thêm sản phẩm chính
    const handleOpenAdd = () => {
        setModalType('add');
        setFormData({
            id: '',
...
normalized_old_helpers = old_open_helpers.replace("\r\n", "\n")
normalized_new_helpers = new_open_helpers.replace("\r\n", "\n")

if normalized_old_helpers in code:
    code = code.replace(normalized_old_helpers, normalized_new_helpers)
    print("Updated handleOpenAdd and handleOpenEdit.")
else:
    print("handleOpenAdd/handleOpenEdit already updated or not matched.")

# 2. Update Action Buttons in the table (remove Chi tiết)
old_buttons = """                                                    <button
...
normalized_old_buttons = old_buttons.replace("\r\n", "\n")
normalized_new_buttons = new_buttons.replace("\r\n", "\n")

if normalized_old_buttons in code:
    code = code.replace(normalized_old_buttons, normalized_new_buttons)
    print("Removed Chi tiết button.")
else:
    print("Chi tiết button already removed or not matched.")

# 3. Robust Line-based replacement of Modal code
lines = code.split("\n")

# Find the index of the start comment: {/* Modal Dialog Thêm/Sửa sản phẩm chính */}
start_line_idx = -1
for idx, line in enumerate(lines):
    if "{/* Modal Dialog Thêm/Sửa sản phẩm" in line:
        start_line_idx = idx
        break
...
# Find the end of the file indices (last 3 lines containing layout tags)
end_line_idx = -1
for idx in range(len(lines) - 1, -1, -1):
    if "</div>" in lines[idx] and idx < len(lines) - 2:
...
print(f"Modal section lines: {start_line_idx} to {end_line_idx}")

# The new unified modal code:
new_modal_code = """            {/* Modal Dialog Thêm/Sửa sản phẩm (Thông tin chung, Ảnh phụ, Biến thể) */}
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
                    <div className={`bg-white w-full rounded-3xl shadow-xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200 transition-all ${
                        modalType === 'edit' && activeTabInModal !== 'basic' ? 'max-w-4xl' : 'max-w-xl'
                    }`}>
                        {/* Modal Header */}
                        <div className="px-6 py-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                            <div>
                                ...
                                {modalType === 'edit' && (
                                    <p className="text-xs text-gray-500 font-semibold mt-0.5">{formData.name}</p>
                                )}
                            </div>
                            <button
                                type="button"
                        ...
                        {activeTabInModal === 'basic' ? (
                            /* TAB 1: THÔNG TIN CHUNG (FORM CHÍNH) */
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-6 flex flex-col">
                                ...
                                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4 overflow-y-auto max-h-[40vh] pr-1">
                                ...
                                <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end -mx-6 -mb-6">
                                    <button
                                        onClick={() => setShowModal(false)}
                                        className="px-5 py-2.5 bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold rounded-xl text-xs transition-colors cursor-pointer"
                                    >
                                        Đóng lại
                                    </button>
                                </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-6 flex flex-col">
                                <form onSubmit={handleAddVariant} className="bg-gray-50/50 border border-gray-100 rounded-2xl p-4 space-y-4">
                                    <div className="space-y-4 max-h-[25vh] overflow-y-auto pr-1">
                                        {tempVariants.map((item, index) => (
                                            <div key={index} className="relative bg-white border border-gray-200 rounded-2xl p-4 space-y-3 shadow-sm">
                                                <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                                                    <span className="text-xs font-black text-indigo-600">Biến thể #{index + 1}</span>
                                                ...
                                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 items-start">
                                                    <div>
                                                        ...
                                                        <input
                                                            type="text"
                                                            value={item.color}
                                                            onChange={(e) => handleTempVariantChange(index, 'color', e.target.value)}
                                                            onFocus={() => setFocusedRow({ index, field: 'color' })}
                                                        ...
                                                        />
                                                        {focusedRow.index === index && focusedRow.field === 'color' && (
                                                                ...
                                                                {['Đen', 'Trắng', 'Xám', 'Xanh', 'Đỏ', 'Hồng', 'Vàng'].map((col) => {
                                                                    ...
                                                                    return (
                                                                        <button
                                                                            key={col}
                                                                            type="button"
                                                                            onClick={() => handleTempVariantChange(index, 'color', col)}
                                                    ...
                                                    <div>
                                                        ...
                                                        <input
                                                            type="text"
                                                            value={item.size}
                                                            onChange={(e) => handleTempVariantChange(index, 'size', e.target.value)}
                                                            onFocus={() => setFocusedRow({ index, field: 'size' })}
                                                            placeholder="S, M, L, 39, 40..."
                                                            className="w-full px-3 py-2 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 text-xs bg-white"
                                                        />
                                                        {focusedRow.index === index && focusedRow.field === 'size' && (
                                                            <div className="mt-1 flex flex-wrap gap-1">
                                                                {['S', 'M', 'L', 'XL', 'XXL', '38', '39', '40', '41', '42', '43', '44', 'Free'].map((sz) => {
                                                                    const isActive = item.size.split('.').includes(sz);
                                                                    return (
                                                                        <button
                                                                            key={sz}
                                                                            type="button"
                                                                            onClick={() => {
                                                                                let currentSizes = item.size ? item.size.split('.') : [];
                                                                                currentSizes = currentSizes.filter(s => s.trim() !== '');
                                                                                if (sz === 'Free') {
                                                                                    currentSizes = ['Free Size'];
                                                                                } else {
                                                                                    currentSizes = currentSizes.filter(s => s !== 'Free Size');
                                                                                ...
                                                                                handleTempVariantChange(index, 'size', currentSizes.join('.'));
                                                                            }}
                                                                            className={`px-1.5 py-0.5 rounded text-[9px] font-bold border transition-all cursor-pointer ${
                                                                                isActive
                                                                                    ? 'bg-indigo-600 border-indigo-600 text-white shadow-sm'
                                                    ...
                                                    <div>
                                                        <label className="block text-[11px] font-semibold text-gray-500 mb-1">Giá phụ thu</label>
                                                        <input
                                                            type="number"
                                                            value={item.price}
                                                            onChange={(e) => handleTempVariantChange(index, 'price', e.target.value)}
                                                    ...
                                                    <div>
                                                        ...
                                                        <input
                                                            type="number"
                                                            value={item.stock}
                                                            onChange={(e) => handleTempVariantChange(index, 'stock', e.target.value)}
                                                            required
                                                            placeholder="0"
                                                            className="w-full px-3 py-2 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 text-xs bg-white"
                                                        />
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center border-t border-gray-100 pt-3">
                                                    <div>
                                                        <label className="block text-[11px] font-semibold text-gray-500 mb-1">Ảnh biến thể</label>
                                                        <div className="flex gap-2">
                                                            <input
                                                                type="text"
                                                                value={item.image}
                                                                onChange={(e) => handleTempVariantChange(index, 'image', e.target.value)}
                                                    ...
                                                    <div className="flex justify-between items-center">
                                                        {item.image ? (
                                                            <div className="relative w-12 h-12 rounded-lg overflow-hidden border border-gray-200 shadow-sm">
                                                                ...
                                                                <button
                                                                    type="button"
                                                                    onClick={() => handleTempVariantChange(index, 'image', '')}
                                    ...
                                    <div className="bg-indigo-50/70 border border-indigo-100 rounded-2xl p-4 mb-4 flex flex-col gap-3">
                                        <div className="flex justify-between items-center">
                                            <span className="text-xs font-bold text-indigo-900">
                                                Đang chọn <span className="text-rose-600">{selectedVariantIds.length}</span> biến thể
                                            </span>
                                            <div className="flex gap-2">
                                                <button
                                                    type="button"
                                                ...
                                                >
                                                    {showBulkEditForm ? '✕ Đóng Panel' : '✏️ Sửa hàng loạt'}
                                                </button>
                                                <button
                                                    type="button"
                                        ...
                                        {showBulkEditForm && (
                                            <div className="bg-white border border-indigo-100 rounded-xl p-3 grid grid-cols-2 md:grid-cols-4 gap-3 items-end">
                                                <div>
                                                    <label className="block text-[10px] font-semibold text-gray-500 mb-1">Màu sắc mới</label>
                                                    <input
                                                        type="text"
                                                        value={bulkEditData.color}
                                                        onChange={(e) => setBulkEditData(prev => ({ ...prev, color: e.target.value }))}
                                                        placeholder="Đen, Trắng..."
                                                        className="w-full px-2.5 py-1.5 border border-gray-200 rounded-lg text-xs bg-white focus:outline-none"
                                                    />
                                                </div>
                                                <div>
                                                    <label className="block text-[10px] font-semibold text-gray-500 mb-1">Size mới</label>
                                                    <input
                                                        type="text"
                                                        value={bulkEditData.size}
                                                        onChange={(e) => setBulkEditData(prev => ({ ...prev, size: e.target.value }))}
                                                        placeholder="S, M, L..."
                                                        className="w-full px-2.5 py-1.5 border border-gray-200 rounded-lg text-xs bg-white focus:outline-none"
                                                    />
                                                </div>
                                                <div>
                                                    <label className="block text-[10px] font-semibold text-gray-500 mb-1">Giá phụ thu mới</label>
                                                    <input
                                                        type="number"
                                                ...
                                                <div className="flex gap-2">
                                                    <div className="flex-1">
                                                        <label className="block text-[10px] font-semibold text-gray-500 mb-1">Tồn kho mới</label>
                                                        <input
                                                            type="number"
                                ...
                                <div className="border border-gray-100 rounded-2xl overflow-hidden bg-white shadow-sm overflow-x-auto max-h-[35vh]">
                                    <table className="w-full text-left border-collapse min-w-[700px]">
                                        <thead>
                                            <tr className="bg-gray-50 text-gray-500 uppercase text-[10px] tracking-wider border-b border-gray-100 font-bold">
                                                <th className="p-3 w-10 text-center">
                                                    <input
                                                        type="checkbox"
                                                        checked={extraVariants.length > 0 && selectedVariantIds.length === extraVariants.length}
                                        ...
                                        <tbody className="divide-y divide-gray-100 text-xs">
                                            {extraVariants.length === 0 ? (
                                                <tr>
                                                    <td colSpan="8" className="p-4 text-center text-gray-400">Chưa cấu hình biến thể nào cho sản phẩm.</td>
                                                </tr>
                                            ) : (
                                                extraVariants.map((v) => {
                                                    ...
                                                    return (
                                                        <tr key={v.id} className={`hover:bg-gray-50/50 transition-colors ${isSelected ? 'bg-indigo-50/20' : ''}`}>
                                                                    ...
                                                                    <div className="flex gap-1.5 items-center">
                                                                        {editingVariantData.image ? (
                                                                            <div className="relative w-8 h-8 rounded-md overflow-hidden border">
                                                                                <img src={getImageUrl(editingVariantData.image)} className="w-full h-full object-cover" />
                                                                                <button
                                                                                    type="button"
                                                                                    onClick={() => setEditingVariantData(prev => ({ ...prev, image: '' }))}
                                                                            ...
                                                                            <label className="bg-gray-100 hover:bg-gray-200 text-gray-700 px-2 py-1 rounded text-[9px] font-bold cursor-pointer transition-colors flex items-center justify-center">
                                                                                ...
                                                                                <input
                                                                                    type="file"
                                                                                    accept="image/*"
                                                                                    onChange={handleEditingVariantImageUpload}
                                                                                    className="hidden"
                                                                                />
                                                                            </label>
                                                                        )}
                                                                        <input
                                                                            ...
                                                                            onChange={(e) => setEditingVariantData(prev => ({ ...prev, image: e.target.value }))}
                                                                            placeholder="URL..."
                                                            ...
                                                            <td className="p-3 font-semibold text-gray-700">
                                                                {isEditing ? (
                                                                    <input
                                                                        ...
                                                                        onChange={(e) => setEditingVariantData(prev => ({ ...prev, color: e.target.value }))}
                                                            ...
                                                            <td className="p-3 font-semibold text-gray-700">
                                                                {isEditing ? (
                                                                    <input
                                                                        ...
                                                                        onChange={(e) => setEditingVariantData(prev => ({ ...prev, size: e.target.value }))}
                                                            ...
                                                            <td className="p-3 text-gray-700 font-medium">
                                                                {isEditing ? (
                                                                    <input
                                                                        ...
                                                                        onChange={(e) => setEditingVariantData(prev => ({ ...prev, stock: e.target.value }))}
...
# Reconstruct everything between start_line_idx and end_line_idx + 1
final_lines = lines[:start_line_idx] + [new_modal_code] + lines[end_line_idx + 1:]
final_code = "\n".join(final_lines)

# Write to file using OS-specific newlines
with open(file_path, "w", encoding="utf-8", newline="\r\n") as f:
    f.write(final_code)

print("Unified tabbed modal replaced successfully!")
