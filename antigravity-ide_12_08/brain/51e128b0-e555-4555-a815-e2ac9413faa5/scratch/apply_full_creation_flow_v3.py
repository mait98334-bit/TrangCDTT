...
if old_open_add in text:
    text = text.replace(old_open_add, new_open_add)
...
old_submit = """    if (res.success) {
      if (modalType === "add") {
        alert(
          "Thêm sản phẩm mới thành công! Bạn có thể tiếp tục thêm Ảnh phụ và Biến thể ở các tab bên trên.",
        );
...
if old_submit in text:
    ...
    print("Updated handleSubmit successfully!")
else:
    print("WARNING: handleSubmit not found!")

# --- Step 3: Update handleExtraImageUpload ---
old_upload = """      if (data.success) {
        // Thêm vào database
...
# --- Step 4: Update handleAddExtraImageUrl ---
...
if old_add_url in text:
    text = text.replace(old_add_url, new_add_url)
...
if old_delete_extra in text:
    text = text.replace(old_delete_extra, new_delete_extra)
...
if old_add_var in text:
    text = text.replace(old_add_var, new_add_var)
...
if old_del_var in text:
    text = text.replace(old_del_var, new_del_var)
...
if old_sel_all in text:
    text = text.replace(old_sel_all, new_sel_all)
...
if old_bulk_del in text:
    text = text.replace(old_bulk_del, new_bulk_del)
...
old_save_edit = """  const saveEditingVariant = async (id) => {
    const res = await fetchApi(`/products/variants/${id}`, {
      method: "PUT",
      body: JSON.stringify({
...
new_save_edit = """  const saveEditingVariant = async (id) => {
    if (modalType === "add") {
      setNewProductVariants((prev) =>
        prev.map((v) => {
          if (v.id === id) {
...
if old_bulk_edit in text:
    text = text.replace(old_bulk_edit, new_bulk_edit)
    print("Updated applyBulkEdit successfully!")
else:
    print("WARNING: applyBulkEdit not found!")

# Write back in binary UTF-8 with unix endings, Prettier will convert to CRLF
...
print("Page layout update process finished successfully!")
